...
float	update_freq(t_notes note)
{
	float	stride;

	stride = note.frequency * SAMPLE_DURATION;
    return (stride);
}

void    update_triangle(float *signal, Oscillator *wave, int cur_note, t_notes *track, int sample_count)
{
    size_t  t;
    ...
    {
		if (track[cur_note].duration * SAMPLE_RATE < sample_count)
		{
			cur_note++;
			wave->phase_stride = update_freq(track[cur_note]);
		}
		if (track[cur_note].frequency == 0.0f)
		{
			wave->phase = 0.0f;
			wave->phase_stride = 0.0f;
		}
		wave->phase_stride = track[cur_note].frequency * SAMPLE_DURATION;
...
void	update_saw(float *signal, Oscillator *wave, int cur_note, t_notes *track, int sample_count)
{
    size_t  t;
...
void    update_square(float *signal, Oscillator *wave, int cur_note, t_notes *track, int sample_count)
{
    size_t  t;
...
void	update_sine(float *signal, Oscillator *wave, int cur_note, t_notes *track, int sample_count)
{
    size_t  t;
...
void	player(t_notes **song, int *tracks)
{
	...
	while (!WindowShouldClose())
	{
		...
		if (IsAudioStreamProcessed(stream))
		{
			...
			while (i < am_tracks)
			{		
				sample_count = loop_count * STREAM_BUFFER_SIZE;
				if (tracks[i] == 4)
					update_sine(wave_signal[i], &wave[i], cur_note[i], song[i], sample_count);
				if (tracks[i] == 1)
					update_square(wave_signal[i], &wave[i], cur_note[i], song[i], sample_count);
				if (tracks[i] == 2)
					update_saw(wave_signal[i], &wave[i], cur_note[i], song[i], sample_count);
				if (tracks[i] == 3)
					update_triangle(wave_signal[i], &wave[i], cur_note[i], song[i], sample_count);
				j = 0;
				while (j < 1024)
				{
